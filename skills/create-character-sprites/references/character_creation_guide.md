...
Use the reference as the identity source. Record hair, clothing, palette, props, body-side asymmetries, scale, and prohibited changes (ensure visual effects like fire or glowing auras are explicitly removed). Describe style with observable traits such as painterly volume, soft gradients, readable silhouette, restrained outlines, and game-scale detail.
...
- The body baseline differs by at most 2 px after normalization; held props must not influence the measurement. Narrow the body range when needed.
